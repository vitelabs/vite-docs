...
## Overview
ViteX API enables users to trade on ViteX DEX without exposing private keys.
ViteX API is categorized into trading API and market trends API. 
Trading API (also known as private API) requires authentication and authorization, and provides functions such as placing and canceling orders. 
...
To use ViteX Private REST API, you must authorize it on [Trade Delegation](https://x.vite.net/tradeTrust)  first in order for ViteX API to trade on your behalf. You need to fill in the delegation address, which is generated by the API service when you applied for the API Key on [ViteX API](https://x.vite.net/tradeOpenapi). You DO NOT need to provide your private key or mnemonic phrase.

Step 1: Create API Key on [ViteX API](https://x.vite.net/tradeOpenapi)
![create-API-key](./create-API-key.png) 

Step 2: Connect Vite Wallet
![API-connect-wallet](./API-connect-wallet.png) 
...
![API-add-trading-pairs](./API-add-trading-pairs.png) 
...
![API-lock-Quota](./API-lock-Quota.png) 
:::
...
Private API requires signature authentication by **API Key** and **API Secret**, which you can apply for on [ViteX API](https://x.vite.net/tradeOpenapi). Please note that API Key and API Secret are both case sensitive.
