...
ViteX API is categorized into trading API and market trends API. Trading API (also known as private API) requires authentication and authorization, and provides functions such as placing and canceling orders. 
...
2 | Received | Order received by ViteX smart contract but not yet dispatched into matching engine
...
To use ViteX Private REST API, you must authorize it at [Trade Delegation](https://x.vite.net/tradeTrust) on ViteX first in order for ViteX API service to trade on behalf of you. You should fill in the delegation address, which is generated by the API service when you applied the API Key. You DO NOT need to provide your private key or mnemonic phrase.
...
Private API requires signature authentication by **API Key** and **API Secret**, which you can apply for at [API](https://x.vite.net/tradeOpenapi) on ViteX. Please note that API Key and API Secret are both case sensitive.

Besides parameters defined by specific API methods, 3 additional parameters `key`, `timestamp` and `signature` also need to be included. 
...
Let's place an order through API `/api/v2/order`. Assume we have the following API Key and API Secret:
...
```
Test placing order. The request will not be submitted to the exchange. This API is generally used to verify that the signature is correct.
