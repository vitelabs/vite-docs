...
To use ViteX Private REST API, you must authorize it on [Trade Delegation](https://x.vite.net/tradeTrust) first in order for ViteX API to trade on your behalf. You should fill in the delegation address, which is generated by the API service when you applied the API Key. You DO NOT need to provide your private key or mnemonic phrase.
...
Private API requires signature authentication by **API Key** and **API Secret**, which you can apply for on [API](https://x.vite.net/tradeOpenapi). Please note that API Key and API Secret are both case sensitive.
